...
import (
	...
	"syscall"
	"time"
	clientSet "vm-controller/pkg/client/clientset/versioned"
	informers "vm-controller/pkg/client/informers/externalversions"
...
func main() {
	...
	virtualMachineClient, err := clientSet.NewForConfig(cfg)
	if err != nil {
	...
	virtualMachineInformerFactory := informers.NewSharedInformerFactory(virtualMachineClient, time.Second*30)

	// 实例化自定义控制器
	controller := NewController(virtualMachineInformerFactory.Cloud().V1().VirtualMachines())
